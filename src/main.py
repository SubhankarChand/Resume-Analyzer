...
with col1:
    ...
    jd_option = st.radio("Input:", ["Paste Text", "Upload PDF"], horizontal=True)
...
if st.button("Analyze Now", type="primary", use_container_width=True):
    if st.session_state.jd_text and uploaded_file:
        with st.spinner("Processing..."):
            ...
            if not user_name:
                user_name = [line.strip() for line in raw_resume.split('\n') if line.strip()][0]
            ...
            tech_m, soft_m = categorize_keywords(matched)
            tech_miss, soft_miss = categorize_keywords(missing)
            ...
            tab1, tab2, tab3 = st.tabs(["✅ Matched", "❌ Skill Gap", "📥 Report"])
            with tab1:
            ...
            with tab3:
                ...
                st.download_button("Download Official PDF Report", pdf_data, 
